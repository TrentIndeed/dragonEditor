...
export function sfxPlacementsToTimelineClips(placements: SFXPlacement[]): TimelineClip[] {
  return placements.filter((p) => p.accepted === true).map((p) => ({ id: `sfx-${p.id}`, trackType: 'sfx' as const, name: p.sfxName, startTime: p.time, duration: p.duration, sourceOffset: 0, color: '#50B0D0' }));
}
